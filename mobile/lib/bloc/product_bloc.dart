...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  ProductBloc() : super(ProductInitial()) {
    ...
    on<EditProduct>((event, emit) {
      try {
        final index =
            _products.indexWhere((p) => p.name == event.updatedProduct.name);
        if (index != -1) {
      ...
      try {
        ...
        if (index != -1) {
          if (_products[index].stock > 0) {
            ...
            emit(const ProductOperationFailure(
...
final List<Map<String, dynamic>> products = [
  {
    ...
    'price': 39.00,
    'quantity': 0,
    'imageUrl': 'assets/Productlist/icedcaramel.jpg',
  },
  {
    'name': 'Don Darko',
    'price': 39.00,
    'quantity': 0,
    'imageUrl': 'assets/Productlist/dondarko.jpg',
  },
